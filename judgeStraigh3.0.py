...
添加大小王为癞子             ------->done
可判别是否同花顺             -------->done
后续考虑随机生成任意张牌，并符合扑克牌规则（四种花色，大小王）4.0
...
class Card:
    ...
    def __repr__(self):
        return '({},{})'.format(self.point,self.suit)
...
if __name__ == '__main__':
    ...
    card5 = Card(5,'Heart')
    card6 = Card(6,'Heart')
    listofcard = [card2,card3,card4,card5,card6]
    flag = judgeStraigh(listofcard)
    if flag == 0:
        print('{} 不是一个顺子...'.format(listofcard))
    elif flag == 1:
        print('{} 是一个顺子,但不是同花顺。'.format(listofcard))
    elif flag == 2:
        print('{} 是一个同花顺，恭喜您！'.format(listofcard))
    else:
        print('不可预料的情况发生了？？？')
